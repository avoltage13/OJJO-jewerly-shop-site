...
const minPics = document.querySelectorAll('[id^="cardBtn_"]');

minPics.forEach(replacePic => {
    replacePic.addEventListener('click', () => {
        const picContent = replacePic.querySelector('picture');
        bigPic.innerHTML = picContent.outerHTML;
    });
});

bigPic.addEventListener('click', () => {
    bigPic.classList.toggle('card__big-picture-expand-button--opend');
    body.classList.toggle("noscroll");
});